...
/**
 * Final, accurate parser for the Adobe PDF CSV file
 * This parser correctly identifies brands and extracts all vehicle data
 */

// Function to parse the Adobe CSV with accurate brand detection
function parseAdobeCSVFinal(filePath) {
  const content = fs.readFileSync(filePath, 'utf8');
  const lines = content.split('\n');
  
  console.log('🔋 ILV FULMEN ENDURANCE Adobe CSV Final Parser\n');
  ...
  // Known brand names to help with detection
  const knownBrands = [
    'ALFA ROMEO', 'AUDI', 'BMW', 'CHEVROLET', 'CHRYSLER', 'CITROËN', 'DACIA',
    'FIAT', 'FORD', 'HONDA', 'HYUNDAI', 'IVECO', 'JEEP', 'KIA', 'LANCIA',
    'LAND ROVER', 'MERCEDES-BENZ', 'MINI', 'MITSUBISHI', 'NISSAN', 'OPEL',
    'PEUGEOT', 'RENAULT', 'SEAT', 'SKODA', 'SMART', 'SUZUKI', 'TOYOTA',
    'VOLKSWAGEN', 'VOLVO'
  ];
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (currentBrand && (line.includes('Diesel') || line.includes('Petrole'))) {
      const entry = parseVehicleLineFinal(line, currentBrand, i + 1);
      if (entry) {
...
// Function to parse a single vehicle line with improved logic
function parseVehicleLineFinal(line, brand, lineNumber) {
  ...
  for (let j = 0; j < parts.length; j++) {
    ...
    if ((part.includes('JTD') || part.includes('TDI') || part.includes('TFSI') || 
         ...
         part.includes('HDi') || part.includes('TDCi') || part.includes('dCi') ||
         part.includes('Tce') || part.includes('VTi') || part.includes('THP') ||
         part.includes('PureTech') || part.includes('BlueHDi') || part.includes('Multijet')) &&
        !motorisation) {
      motorisation = part;
      break;
    }
...
function saveToCSV(data, outputPath) {
  ...
  console.log(`✅ Final data saved to: ${outputPath}`);
}
...
function main() {
  ...
  // Parse the file with accurate brand detection
  const rawData = parseAdobeCSVFinal(inputPath);
  ...
  // Save cleaned data
  const outputPath = path.join(__dirname, 'ILV-FULMEN-ENDURANCE_ADOBE_FINAL.csv');
  saveToCSV(cleanedData, outputPath);
  
  console.log('\n✅ Final Adobe CSV parsing complete!');
  console.log('\n📝 Next steps:');
  console.log('1. Review the final CSV file: ILV-FULMEN-ENDURANCE_ADOBE_FINAL.csv');
