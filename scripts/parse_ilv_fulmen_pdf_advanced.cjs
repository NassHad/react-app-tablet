...
/**
 * Advanced parser for ILV FULMEN ENDURANCE PDF
 * Handles the specific table-like format of this PDF
 */
...
// Function to parse the PDF using a more sophisticated approach
function parsePDFAdvanced(text) {
  ...
  while (i < lines.length) {
    ...
    if ((line.includes('Diesel') || line.includes('Petrole') || line.includes('Essence') || 
      ...
      // Extract dates from the same line or next lines
...
async function main() {
  ...
  console.log('🔋 ILV FULMEN ENDURANCE PDF Parser (Advanced)\n');
  ...
  // Parse the PDF using advanced method
  console.log('🔍 Parsing PDF with advanced method...');
  const rawData = parsePDFAdvanced(text);
  ...
  // Save clean data
  const outputPath = path.join(__dirname, 'ILV-FULMEN-ENDURANCE_PDF_ADVANCED.csv');
  saveToCSV(cleanedData, outputPath);
  
  console.log('✅ Advanced PDF parsing complete!');
  console.log('\n📝 Next steps:');
  console.log('1. Review the clean CSV file: ILV-FULMEN-ENDURANCE_PDF_ADVANCED.csv');
