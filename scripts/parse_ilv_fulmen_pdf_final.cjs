...
/**
 * Final parser for ILV FULMEN ENDURANCE PDF
 * Handles the specific format with proper fuel type and date separation
 */
...
// Function to parse the PDF with proper fuel/date separation
function parsePDFFinal(text) {
...
async function main() {
  ...
  console.log('🔋 ILV FULMEN ENDURANCE PDF Parser (Final)\n');
  ...
  // Parse the PDF using final method
  console.log('🔍 Parsing PDF with final method...');
  const rawData = parsePDFFinal(text);
  ...
  // Save clean data
  const outputPath = path.join(__dirname, 'ILV-FULMEN-ENDURANCE_PDF_FINAL.csv');
  saveToCSV(cleanedData, outputPath);
  
  console.log('✅ Final PDF parsing complete!');
  console.log('\n📝 Next steps:');
  console.log('1. Review the clean CSV file: ILV-FULMEN-ENDURANCE_PDF_FINAL.csv');
